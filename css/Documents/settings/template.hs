...
deleteMin heap
  = mergeHeaps heap' minRootHeap 
  where
    minRootTree = head $ dropWhile (\t -> value t /= extractMin heap) heap
    minRootHeap = reverse $ children minRootTree
    heap' = filter (\t -> t /= minRootTree) heap
...
binSort 
  = binSort' . heap 
  where 
    heap = foldl (flip insert) []
    binSort' []  = []
    binSort' as' = (extractMin as') : (binSort' $ deleteMin as')
...
toBinary h
  = reverse $ toBinary' [0..] h
  where
    toBinary' _ [] = []
    toBinary' (n:ns) t'@(t:ts)
      | n == rank t = 1 : (toBinary' ns ts)
      | otherwise   = 0 : (toBinary' ns t')
