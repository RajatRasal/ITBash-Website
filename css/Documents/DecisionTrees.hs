...
remove _ []
  = []
remove k kv
  = (takeWhile checkFst kv) ++ (tail $ dropWhile checkFst kv)
  where
    checkFst = (flip $ (/=) . fst) k
...
lookUpAtt attName header row 
  = head $ filter (`elem` att) row 
  where 
    att = lookUp attName header

removeAtt :: AttName -> Header -> Row -> Row
removeAtt attName header row
  = (takeWhile (/= att) row) ++ (tail $ dropWhile (/= att) row)
  where
    att = lookUpAtt attName header row

addToMapping :: Eq a => (a, b) -> [(a, [b])] -> [(a, [b])]
addToMapping (k,v) [] 
  = [(k, [v])]
addToMapping kv@(k,v) (kv'@(k',v'):kvs)
  | k == k'   = (k',v:v'):kvs
  | otherwise = kv' : (addToMapping kv kvs)
