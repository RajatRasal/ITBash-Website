...
-- This solution works since a tag cannot have 2 attributes of the 
-- same name. 
getAttribute :: String -> XML -> String
getAttribute att (Element _ attrs _) 
  = foldl (\x (att', val) -> if att' == att then val else x) "" attrs
getAttribute _ _  
  = "" 

getChildren :: String -> XML -> [XML]
getChildren name (Element _ _ children) 
  = foldr (\elem' x -> if checkChild elem' then (elem':x) else x) [] children 
  where 
    checkChild (Element name' _ _)
      = name == name'
    checkChild _
      = False
getChildren _ _
...
getValue (Element _ _ children) 
  = Text (getValue' children) 
  where 
   getValue' ((Text x):xs) 
     = x ++ (getValue' xs)
   getValue' ((Element _ _ children'):xs) 
     = (getValue' children') ++ (getValue' xs)
   getValue' _ 
     = ""
...
parseAttributes str 
  | c == '>'  = ([], str')
  | otherwise = ((attr, val):attrs, strs') 
  where
    strs@(c:str')      = skipSpace str
    (attr, rest)  = parseName str'
    (_, (_:rest'))  = break (== '"') rest
    (val, rest'')   = break (== '"') rest'
    (_, (_:rest''')) = break (== '"') rest''
    (attrs, strs') = parseAttributes rest'''
...
parse s
  ...
  where
    ...
    parse' "" (e@(Element name _ _):_) 
      = getChild name e
    parse' str@(x:ys'@(y:ys)) stack 
      | x == '<' && y == '/' = parse' rest (popAndAdd stack)
      | x == '<' && y /= '/' = parse' rest'' (newElem:stack)
      | otherwise            = parse' rest''' (addText text stack)
      where
        (_, rest)       = break (== '>') ys
        (name, rest')   = break (== ' ') rest 
        (attrs, rest'') = parseAttributes rest' 
        newElem         = Element name attrs []
        (text, rest''') = break (== '<') str
