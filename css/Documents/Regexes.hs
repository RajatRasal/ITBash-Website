...
-- (Start state, end state, char or empty)
type Transition = (State, State, Label)

-- (Unique start state, [Terminal state], [Transition])  
...
-- If a state is a terminal state in the automation.
isTerminal :: State -> Automaton -> Bool
isTerminal 
  = (. terminalStates) . elem 

-- A list of all the transitions emanating from a given automaton
...
accepts a@(s', _, _) str' 
  ...
  where 
    ...
    accepts' s str@(_:strs)
      | (isTerminal s a) && null str = True
      | or $ map (try str) ts     = accepts' t strs
      | otherwise                  = False 
      where 
        ts        = transitionsFrom s a
        (_, t, _) = head $ dropWhile (not . (try str)) ts
